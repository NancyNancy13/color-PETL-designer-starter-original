...
// createFourCopies //// it copy the color from the original color
function createFourCopies(original) {
  const copies = []; // need an array to copy the color
  for (let i = 0; i < 4; i++) {
...
function setHarmony(harmony) {
  ...
  // switch is baiscally alternative fod the if else statement
  switch (harmony) {
    case "analog":
      break;
  }
...
function calculateHarmony() {
  const indexes = [1, 2, 4, 5]; // number of the color having in the html
  ...
  console.log("caluclateHarmony base:", base);
